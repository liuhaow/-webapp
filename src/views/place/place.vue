	...
	<div class="xiadan-main">
		...
		<div class="maininfo">
			<div class="product-name">
				...
				<p>{{xiadanbuy.all.title}}</p>
			</div>
			<div class="game-area">
				<h1 class="commen-sty">游戏区服</h1>
				<p><span>{{xiadanbuy.all.gameName}}</span>/<span>{{xiadanbuy.all.game_area_name}}</span>/<span></span></p>
			</div>
			<div class="unit-price">
				<h1 class="commen-sty">单价</h1>
				<span>&yen;{{xiadanbuy.all.gold_price}}</span>
			</div>
			<div class="xiadan-numb">
				<h1 class="commen-sty">数量</h1>
				<ul>
					<li>
						<span class="btn-left" @click="subtract()" :class="{ 'active1':style1 }">-</span>
					</li>
					<li><span class="zk-input">{{count}}</span></li>
					<li>
						<span class="btn-right" @click="add()" :class="{ 'active2':style2 }">+</span>
					</li>
				</ul>
		...
		<div class="mian-footer">
			<h1>合计 <span ref="pice" v-show="!value">{{xiadanbuy.all.gold_price*count}}</span> <span ref="pices" v-show="value">{{xiadanbuy.all.gold_price*count+xiadanbuy.all.unit_price}}</span></h1>
...
<script>
	...
	import { buyjsdbData } from '@/api/api'
		...
		methods: {
			...
			payInfo() {
				...
				let data = {
					...
					gameTitle: this.xiadanbuy.all.title,
					gameAreaId: this.xiadanbuy.all.game_area_id,
					token: this.getUserData.token,
					userId: this.getUserData.id,
					gameId: this.yibiqufu.gameId,
					gameGoldId: this.xiadanbuy.all.id,
					num: this.count,
